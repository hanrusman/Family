...
import { MODEL_COLORS, MODEL_LABELS } from '../../utils/weatherColors';
import { formatTemp, formatWind } from '../../utils/weatherFormat';
...
const VARIABLE_KEYS = ['temperature', 'feelsLike', 'precipitation', 'wind'];

function formatXTick(time, range) {
...
export default function MultiModelChart({ forecast, enabledModels, allModels, isEnabled, onToggle }) {
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 0; i < times.length; i += step) {
      ...
      for (const [modelId, hourly] of models) {
        const dataArray = hourly[varConfig.dataKey];
